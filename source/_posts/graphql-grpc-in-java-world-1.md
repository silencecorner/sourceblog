...
以目前情况来讲，不需要多语言调用，即使出现多言调用的情况也可以，也不影响正常调用，只是缺少验证而已，再不济也可以自己实现嘛！
#### 修改proto
```proto
...
```
### 总结
介绍graphql、gprc in java world的一些问题，一些intergration的思路，心、新特性参数验证，下一篇介绍使用graphql结合field mask做单项更新！